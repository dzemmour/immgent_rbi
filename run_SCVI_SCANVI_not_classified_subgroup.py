...
while ((percentage > 1) and (difference > 1.5)):
    ...
    label_counts = mdata.obs["level2_final"].value_counts()
    drop = label_counts[label_counts < 5].index.tolist()     # labels with < 5 cells are considered drop     
    mdata.obs.loc[mdata.obs["level2_final"].isin(drop), "level2_final"] = "not classified"

    mdata.obs["level2_final"] = mdata.obs["level2_final"].astype("category")
    
    n_labeled = (mdata.obs["level2_final"] != "not classified").sum()
    ...
    level2_model.train(30, train_size=0.9, validation_size=0.1, batch_size=128, datasplitter_kwargs={"drop_last": True})
