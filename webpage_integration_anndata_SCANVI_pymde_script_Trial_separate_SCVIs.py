...
## start for loop- try to only use 1 from the start

## not just level1 and level2
## split into abT and gdT again ... abT level1 and level2, gdT level2
...
mde_ref_embedding = mde_ref_embedding.loc[mde_ref_embedding.index.intersection(mdata.obs.index)]
...
## abT
## level1 mde
mde_ref_embedding_level1_abT = mde_ref_embedding_level1.loc[mde_ref_embedding_level1.index.intersection(latent_df_scvi_abT.index)]
rownames = mde_ref_embedding_level1_abT.index.tolist()  # Convert index to list
index_positions = [latent_df_scvi_abT.index.get_loc(item) for item in rownames if item in latent_df_scvi_abT.index]
...
incremental_mde_abT = pymde.preserve_neighbors(
    torch.tensor(latent_df_scvi_abT.values, device='cpu'), # dtype=torch.float32, device='cpu'), #device='cuda:0'), #device='cpu'
    embedding_dim=2,
...
level1_abT_mde_df = pd.DataFrame(incremental_mde_abT.X.cpu().numpy(), index = latent_df_scvi_abT.index)

output_file.loc[latent_df_scvi_abT.index, 'allT_MDE1'] =  level1_abT_mde_df[0]
output_file.loc[latent_df_scvi_abT.index, 'level2_MDE1'] =  level1_abT_mde_df[0]
output_file.loc[latent_df_scvi_abT.index, 'allT_MDE2'] =  level1_abT_mde_df[1]
...
output_file.loc[latent_df_scvi_gdT.index, 'allT_MDE1'] =  level1_gdT_mde_df[0]
output_file.loc[latent_df_scvi_gdT.index, 'level2_MDE1'] =  level1_gdT_mde_df[0]
output_file.loc[latent_df_scvi_gdT.index, 'allT_MDE2'] =  level1_gdT_mde_df[1]
output_file.loc[latent_df_scvi_gdT.index, 'level2_MDE2'] =  level1_gdT_mde_df[1]
...
for annotation in level1_annotations:
    ...
    latent_df = level2_abT_latent_df
    if annotation == "gdT":
        latent_df = level2_gdT_latent_df
